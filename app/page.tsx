...
const T = {
  tr: {
    ...
    price: { eye: "Abonelik", title: "Binanıza uygun planı seçin.", lead: "Villa ve işyerinden büyük sitelere, her yapıya uygun esnek fiyatlandırma.", choose: "Başla" },
    ...
    faq: {
      ...
      items: [
        { q: "MobilDiafon nedir, klasik diafondan farkı ne?", a: "MobilDiafon, bina girişini panelsiz ve bulut tabanlı yöneten dijital bir diafon platformudur. Çağrılar telefonunuza gelir; nerede olursanız olun görüntülü görüşür ve kapıyı uzaktan açarsınız." },
        { q: "Kurulum için binada donanım veya panel değişikliği gerekiyor mu?", a: "Hayır. Girişe bir QR afiş asmanız yeterlidir; ziyaretçi kodu okutur, bina doğrulanır ve doğru daireye görüntülü arama başlar. Panel veya kablolama zorunlu değildir." },
        { q: "Ziyaretçi uygulama indirmeden nasıl arama yapıyor?", a: "Ziyaretçi giriş QR kodunu okutur, tarayıcı üzerinden doğrudan görüntülü çağrı başlar. Ziyaretçinin uygulama indirmesine gerek yoktur." },
        { q: "Yönetici sakinleri nasıl ekliyor ve onaylıyor?", a: "Yönetici panelinden daireleri tanımlar, katılım isteği gönderen sakinleri onaylar; güvenlik görevlisi ekleme, çağrı kayıtları ve not bırakma ekranları hazırdır." },
        { q: "Abonelik nasıl işliyor, istediğim zaman iptal edebilir miyim?", a: "Yapı büyüklüğüne göre aylık veya yıllık esnek planlar sunulur. Aboneliğinizi dilediğiniz zaman yönetebilir veya iptal edebilirsiniz." },
        { q: "Tuya akıllı kapı ve cihazlarla uyumlu mu?", a: "Evet. Tuya uyumlu akıllı kapı ve cihazlarla entegre çalışır; kapı açma işlemleri yetki kontrolüyle ilerler." },
      ],
    },
 footer: {
  ...
  en: {
    ...
    price: { eye: "Subscription", title: "Choose the plan that fits your building.", lead: "From single units to large complexes, flexible pricing for every structure.", choose: "Get Started" },
    ...
    faq: {
      ...
      items: [
        { q: "What is MobilDiafon and how is it different from a classic intercom?", a: "MobilDiafon is a cloud-based digital intercom platform that manages building entry without a panel. Calls reach your phone; you talk on video and open the door remotely from anywhere." },
        { q: "Does installation require hardware or panel changes in the building?", a: "No. You just place a QR poster at the entrance; the visitor scans it, the building is verified and a video call starts to the right flat. No panel or wiring is required." },
        { q: "How does a visitor call without installing an app?", a: "The visitor scans the entrance QR code and a video call starts directly in the browser. No app download is needed for visitors." },
        { q: "How does a manager add and approve residents?", a: "From the manager panel you define flats and approve residents who request to join; security guard management, call logs and note-leaving screens are ready to use." },
        { q: "How does the subscription work and can I cancel anytime?", a: "Flexible monthly or yearly plans are offered based on building size. You can manage or cancel your subscription at any time." },
        { q: "Is it compatible with Tuya smart doors and devices?", a: "Yes. It integrates with Tuya-compatible smart doors and devices; door-opening actions run through access control." },
      ],
    },
 footer: {
...
function IconPin() { return (<svg width="16" height="16" viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M12 21s-7-6.5-7-11a7 7 0 0 1 14 0c0 4.5-7 11-7 11Z" stroke="currentColor" strokeWidth="1.8" /><circle cx="12" cy="10" r="2.5" stroke="currentColor" strokeWidth="1.8" /></svg>); }
function IconApp() { return (<svg width="16" height="16" viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M12 3 3 8l9 5 9-5-9-5Z" stroke="currentColor" strokeWidth="1.8" strokeLinejoin="round" /><path d="m3 16 9 5 9-5M3 12l9 5 9-5" stroke="currentColor" strokeWidth="1.8" strokeLinejoin="round" /></svg>); }
...
function AppShowcase() {
  ...
  const go = (dir: number) => setIdx((v) => (v + dir + shots.length) % shots.length);

  // sürükleme (mouse + dokunmatik)
  function onDown(x: number) { drag.current = { x, active: true }; setPaused(true); }
  function onUp(x: number) {
    if (!drag.current.active) return;
    const dx = x - drag.current.x;
    drag.current.active = false;
    if (Math.abs(dx) > 50) go(dx < 0 ? 1 : -1); // sola çek → sonraki
    setPaused(false);
  }
  ...
  return (
    <div
      ...
      onMouseLeave={() => { drag.current.active = false; setPaused(false); }}
        ...
        <div className="hero2-left">
          ...
          <div className="hero2-cta">
            <button className="btn btn-primary" onClick={useMyLocation} disabled={locating}>
              ...
              {locating ? t.heroLoc.locating : "Konumdan Tara"}
            </button>
            <a href="#nasil" className="btn btn-ghost">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" style={{ marginRight: 6, verticalAlign: -2 }}><path d="M8 5v14l11-7z" /></svg>
              Nasıl Çalışır?
            </a>
            <img src="https://cdn.mobildiafon.com/tuya.webp" alt="Tuya uyumlu" className="hero2-tuya" />
          </div>
          {locResult && (
...
<img src="/aws-logo.svg" alt="Amazon Web Services" className="md-partner-logo" />
 ...
 <JourneyScene />
                ...
                return (
                  <article className={`md-price-card ${isFeatured ? "featured" : ""}`} key={p.id}>
                    ...
                    <div className="md-price">
                      {isFree ? lang === "en" ? "Quote" : "Teklif" : `₺${bbFmt(p.monthlyPrice)}/ay`}
                    </div>
                    <p>{detailText}</p>
                    <a href="#bina"
        ...
        <section id="sss" className="md-section md-faq-sec">
          <div className="md-wrap">
            <Reveal variant="fade" className="md-center">
              <span className="md-eyebrow">{t.faq.eye}</span>
              <h2 className="md-title">{t.faq.title}</h2>
            </Reveal>
            <div className="md-faq">
              {t.faq.items.map((it, i) => (
                <details className="md-faq-item" key={i}>
                  <summary>{it.q}<span className="md-faq-ic" aria-hidden="true" /></summary>
                  <div className="md-faq-a">{it.a}</div>
                </details>
              ))}
            </div>
          </div>
  ...
  <div className="md-wrap md-foot-grid">
    ...
    <div className="md-foot-brand">
      ...
      <div className="md-foot-social">
        <a href="https://instagram.com/mobildiafon" aria-label="Instagram" target="_blank" rel="noopener noreferrer">
        ...
        <a href="https://wa.me/905XXXXXXXXX" aria-label="WhatsApp" target="_blank" rel="noopener noreferrer">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
            <path d="M12.04 2C6.58 2 2.13 6.45 2.13 11.91c0 2.1.55 4.05 1.6 5.8L2 22l4.42-1.16a9.9 9.9 0 0 0 5.62 1.72h.01c5.46 0 9.91-4.45 9.91-9.91 0-2.65-1.03-5.14-2.9-7.01A9.86 9.86 0 0 0 12.04 2zm0 18.14h-.01a8.2 8.2 0 0 1-4.18-1.15l-.3-.18-2.62.69.7-2.56-.2-.32a8.2 8.2 0 0 1-1.26-4.39c0-4.54 3.7-8.24 8.25-8.24a8.2 8.2 0 0 1 5.83 2.42 8.2 8.2 0 0 1 2.41 5.83c0 4.55-3.7 8.24-8.25 8.24zm4.52-6.16c-.25-.12-1.47-.72-1.69-.81-.23-.08-.39-.12-.56.13-.16.25-.64.81-.79.97-.14.17-.29.19-.54.06-.25-.12-1.05-.39-1.99-1.23-.74-.66-1.23-1.47-1.38-1.72-.14-.25-.01-.38.11-.51.11-.11.25-.29.37-.43.13-.14.17-.25.25-.41.08-.17.04-.31-.02-.43-.06-.12-.56-1.34-.76-1.84-.2-.48-.4-.42-.56-.43-.14 0-.31-.01-.47-.01a.9.9 0 0 0-.65.31c-.22.25-.86.84-.86 2.05 0 1.21.88 2.38 1 2.54.12.17 1.74 2.66 4.22 3.73.59.25 1.05.41 1.41.52.59.19 1.13.16 1.56.1.48-.07 1.47-.6 1.68-1.18.21-.58.21-1.07.14-1.18-.06-.1-.22-.16-.47-.28z" />
          </svg>
        </a>
      </div>
    </div>
